document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadDoctors() {
    try {
      const response = await fetch(
        'https://mallang-a85bb2ff492b.herokuapp.com/api/doctors'
      );
      if (response.ok) {
      ...
      .map(
        (doctor) => `
                <tr>
                    <td>
                        <img src="${
                          doctor.photoUrl || '/assets/default.jpg'
                        }" alt="${
          doctor.name
        }" width="50" height="50" style="object-fit: cover; border-radius: 8px; background-color: #f0f0f0;">
                    </td>
                    <td>${doctor.name}</td>
                    <td>${doctor.departmentName || '정보 없음'}</td>
                    <td>${doctor.phoneNumber || '정보 없음'}</td>
                    <td>
                        <a href="#" class="btn-update" data-id="${
                          doctor.id
                        }">수정</a>
  ...
  async function addDoctor() {
    ...
    const department = document
      .getElementById('doctor-department')
      .value.trim();
  ...
  async function deleteDoctor(doctorId) {
    try {
      const response = await fetch(
        `https://mallang-a85bb2ff492b.herokuapp.com/api/doctors/${doctorId}`,
        {
          method: 'DELETE',
          headers: {
      ...
      if (response.ok) {
        ...
        alert(
          `의료진 삭제에 실패했습니다. 상태 코드: ${response.status}\n응답 내용: ${errorText}`
        );
      }
    } catch (error) {
  ...
  async function loadVacations() {
    try {
      const response = await fetch(
        'https://mallang-a85bb2ff492b.herokuapp.com/api/vacations'
      );
      if (response.ok) {
  ...
  async function addVacation() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          `휴진 등록에 실패했습니다. 상태 코드: ${response.status}\n응답 내용: ${errorText}`
        );
      }
    } catch (error) {
      ...
      .map(
        (vacation) => `
                <tr>
                    ...
                    <td>
                        <button class="btn-delete-vacation" data-id="${
                          vacation.id
                        }">삭제</button>
                    </td>
                </tr>`
  ...
  async function deleteVacation(vacationId) {
    try {
      const response = await fetch(
        `https://mallang-a85bb2ff492b.herokuapp.com/api/vacations/${vacationId}`,
        {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        alert(
          `휴진 삭제에 실패했습니다. 상태 코드: ${response.status}\n응답 내용: ${errorText}`
        );
      }
    } catch (error) {
  ...
  async function initializePage() {
    ...
    document
      .querySelector('.add-vacation-btn')
      .addEventListener('click', addVacation);
    document
      .querySelector('.add-doctor-btn')
      .addEventListener('click', addDoctor); // 의료진 등록 버튼 이벤트 리스너 추가
