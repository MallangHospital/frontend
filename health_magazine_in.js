...
function renderMagazineDetails(news) {
  // 제목
  document.getElementById("magazine-title").textContent = news.title || "제목 정보 없음";

  // 작성자 및 날짜
  document.getElementById("magazine-writer").textContent = `작성자: ${news.newsWriter || "정보 없음"}`;
  document.getElementById("magazine-date").textContent = `작성일: ${news.createdAt || "날짜 없음"}`;
