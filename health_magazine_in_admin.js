...
async function loadMagazineDetails() {
  ...
  try {
    ...
    document.getElementById(
      'magazine-writer'
    ).textContent = `작성자: ${magazine.newsWriter}`;
    document.getElementById(
      'magazine-date'
    ).textContent = `등록일: ${magazine.regDate}`;
    document.getElementById('magazine-content').innerHTML = magazine.content;

    const magazineImageContainer = document.getElementById(
...
function redirectToEditPage() {
  ...
  // 수정 페이지로 이동
  window.location.href = `magazine_update_admin.html?id=${magazineId}`;
}
